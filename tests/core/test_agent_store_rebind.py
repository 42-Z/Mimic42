...
@pytest.mark.asyncio
async def test_rebind_rejects_a_different_telegram_account() -> None:
    owner_id = uuid4()
    agent_id = uuid4()
    store = InMemoryAgentStore()
    await store.create_from_onboarding(
    ...
    different_account = _authorized_session(owner_id, uuid4(), "new-encrypted-session")
    different_account.phone_number = "+79991111111"

    with pytest.raises(ValueError, match="Telegram account"):
        await store.rebind_telegram_session(agent_id, different_account)

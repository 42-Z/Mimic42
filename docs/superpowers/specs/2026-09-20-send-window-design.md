...
Расхождения со спекой, выявленные кодом и замерами. Они приоритетнее текста выше.

- **Гейт только для групп.** Окно проверяется, когда `event.is_group`. В ЛС ограничений на
  запись нет, а пост канала читают, не отвечая в него: комментарий уходит в связанную
  группу. Иначе бессрочно «закрытый» канал заставил бы выбрасывать посты и убил бы
  комментарии. По той же причине инструменты с `comment_to_msg_id` окно не проверяют.
- **Проверка окна и ход — одна операция.** Они идут под `_dispatch_lock`. Иначе сообщения,
  пришедшие во время хода, проходили гейт при ещё открытом окне, а на отправке упирались в
  окно, закрытое предыдущим ответом: предохранитель отменял ответ, содержимое терялось.
- **Слив перепроверяет окно** и при повторном закрытии откладывает накопленное заново.
- **Статусы событий.** `message.deferred` — `succeeded`; `message.blocked` и
  `message.write_forbidden` — `cancelled`, не `failed`: дашборд считает ошибкой каждое
  `failed`, а кд и запрет писать — не поломка агента.
- **Кэш.** Закрытое окно перечитывается через 60 с (`restricted_ttl`), открытое — через
  300 с. Пока кэш свежий, min-сущность из события не заставляет ходить в сеть.
- **Слот не тратится**, если ветка инструмента вернулась, ничего не отправив.
- **Шапка пачки подобрана замером на самой слабой модели каталога.** Прежняя формулировка
  («ответить можно только ОДНИМ сообщением») читалась как требование отвечать: в чужом
  разговоре модель отвечала 4 из 4 против 2 из 4 без шапки. Нынешняя («…только если что-то
  из этого адресовано тебе») даёт 2 из 8. Пропущенный `reply_to` достраивает рантайм.
- **Уведомление о запрете писать модель не игнорирует** — слабая модель отвечает на любой
  входящий текст (7 из 8; явная просьба промолчать — 6 из 8). Молчание в закрытом чате
  обеспечивает предохранитель рантайма, а не поведение модели.

## Непроверенное на живом Telegram

Не подтверждено, потому что сессия проверяющего аккаунта больше не авторизована
(нужен интерактивный `uv run python -m mimic42.testing.real_tg.login`):

- освобождён ли создатель/админ супергруппы от медленного режима;
- какие апдейты приходят при включении кд и при запрете писать (решает, нужна ли
  подписка на `events.Raw` для мгновенного сброса кэша);
- что отдаёт `GetFullChannel` обычному участнику: приходит ли `slowmode_next_send_date`;
- живые сценарии `tests/real_tg/backend/test_real_slowmode.py` написаны, но не запускались.

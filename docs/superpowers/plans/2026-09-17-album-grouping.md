...
**Внимание:** `bun run build` перезаписывает `.next` и ломает уже запущенный `next dev` (HTML начинает ссылаться на несуществующие чанки, все JS-чанки отдают 404, страницы не гидрируются). Если dev-сервер запущен — либо остановить его перед сборкой, либо после сборки перезапустить и перезагрузить вкладку.
